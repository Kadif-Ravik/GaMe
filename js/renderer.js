...
Renderer.prototype = {
  ...
  clear: function() {
    ...
    for (let x = 1; x < w; x += u) { ctx.moveTo(x, 0); ctx.lineTo(x, h); }
    // draw horizontal grid lines
    for (let y = 1; y < h; y += u) { ctx.moveTo(0, y); ctx.lineTo(w, y); }
  ...
  styles: {
    ...
    fadeIn: function(cell, ctx, unit) {
      let self = this, color = cell.lifeColor();
